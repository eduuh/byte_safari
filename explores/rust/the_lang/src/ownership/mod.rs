/*
  What is ownership?
  A set of rules that governs how rust program manages memory.
  Rust does this by not needing a garbage collector.
  if ownership rules are violated, the program will not compile
  Expectation: Take time for ownership system to click

*/

pub fn ownership_rules() {
    let rule1 = "Each Value in Rust has an owner";
    let rule2 = "There are only be one owner at a time";
    let rule3 = "When the owner goes out of scope, the value will be dropped";
